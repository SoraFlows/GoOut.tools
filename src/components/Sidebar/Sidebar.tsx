...
const Sidebar: React.FC<SidebarProps> = ({ categories, activeCategory, onCategoryClick }) => {
    return (
        <>
            <nav className='after:h-[calc(100vh-65px)] block min-h-screen w-60 flex-row flex-nowrap 
            bg-gray-50 font-semibold sm:bg-background sm:px-6 sm:pb-6'>
                ...
                <div className="flex-start relative z-40 flex h-auto w-full flex-1 flex-col overflow-y-auto overflow-x-hidden rounded pt-4 opacity-100" aria-label="Sidebar">
                    <div className='flex list-none flex-col md:min-w-full md:flex-col'>
                        <div className='flex-none overflow-y-auto pb-12'>
                            ...
                            sm:before:w-[10px] sm:before:rounded-full sm:before:transition-colors">
                                                    <span className="truncate text-sm" onClick={() => onCategoryClick(category.category)}>{category.category}
                                                    </span>
                                                </div>
                                            </div>
